...
describe('src/services/fitment/fitment.service', () => {
    ...
    describe('getManufacturers()', () => {
        ...
        it('Should request manufacturers by default locale', async () => {
            ...
            expect(query).toMatch(/key as id, COALESCE\(name->'de_de', name->'de_de'\) as name, logo as "logoUrl"/);
        });

        it('Should request by query local', async () => {
            await instance.getManufacturers('de', 'en_us');
            const [query] = pgMock.query.getCall(1).args;
            expect(query).toMatch(/key as id, COALESCE\(name->'en_us', name->'de_de'\) as name, logo as "logoUrl"/);
        });
    ...
    describe('getVehiclesById()', () => {
        ...
        const pg = {
            query: sandbox.stub()
                .onFirstCall().resolves({
                    rows: [{ id: 'vehicleId', hsntsnRaw: ['123,456'], maxSpeedKm: 10, fitments: [] }],
                })
                .resolves({ rows: [] }),
        };
